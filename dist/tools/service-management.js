...
export function getServiceManagementTools() {
    return [
        // ── Services ───────────────────────────────────────────────────────────
        {
            ...
            description: 'Get all services from NSP Service Supervision. Supports filtering by name, description, or other criteria.',
            inputSchema: {
                type: 'object',
                properties: {
                    filter: {
                        type: 'string',
                        description: "Filter expression. Examples: \"name='ELINE-001'\", \"description LIKE '%ServiceSup%'\", \"name LIKE '%ELINE%' AND NOT (description LIKE '%test%')\"",
                    },
...
export async function handleServiceManagementTool(name, args, client) {
    ...
    switch (name) {
        ...
        case 'nsp_svc_get_service_sites': {
            const fdn = encodeURIComponent(args.service_fdn);
            if (args.site_fdn) {
                const sFdn = encodeURIComponent(args.site_fdn);
        ...
        case 'nsp_svc_get_service_tunnel_bindings': {
            ...
            if (args.site_fdn) {
                ...
                if (args.tunnel_binding_fdn) {
                    const tbFdn = encodeURIComponent(args.tunnel_binding_fdn);
        ...
        case 'nsp_svc_get_lsp': {
            const fdn = encodeURIComponent(args.lsp_fdn);
        ...
        case 'nsp_svc_get_tunnel': {
            const fdn = encodeURIComponent(args.tunnel_fdn);
