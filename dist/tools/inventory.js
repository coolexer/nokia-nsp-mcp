...
export function getNetworkInventoryTools() {
    return [
        // ── Network Elements ───────────────────────────────────────────────────
        {
            ...
            description: 'Get all Network Elements (NEs) managed by NSP. Supports depth, fields filter, pagination, and time-range filtering.',
            inputSchema: {
                type: 'object',
                properties: {
                    depth: {
        ...
        },
        {
            ...
            description: 'Advanced inventory search using the nsp-inventory:find operation. Supports xpath-like filters across NEs, cards, ports, shelves, LAGs, etc. Use this for complex queries with multiple filter criteria.',
            inputSchema: {
                type: 'object',
                ...
                properties: {
                    xpath: {
                        type: 'string',
                        description: 'XPath expression for the resource to search, e.g. "/nsp-equipment:network/network-element" or "/nsp-equipment:network/network-element/hardware-component/card"',
                    },
                    fields: {
                        type: 'array',
                        items: { type: 'string' },
                    ...
                    filter: {
                        type: 'string',
                        description: 'Filter expression, e.g. "ne-id=\'10.10.10.1\'" or "type=\'7750 SR-12\'" or "admin-state=\'unlocked\'"',
                    },
                    depth: {
                        type: 'number',
        ...
        },
        {
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    port_id: {
                        type: 'string',
                        description: 'URL-encoded port ID, e.g. "shelf%3D1%2FcardSlot%3D1%2Fcard%3D1%2Fport%3Dethernet-1%2F1" or raw "shelf=1/cardSlot=1/card=1/port=1/1/1"',
                    },
                    include_transceiver: {
                        type: 'boolean',
...
export async function handleNetworkInventoryTool(name, args, client) {
    switch (name) {
        case 'nsp_inventory_get_network_elements': {
            const params = {};
            if (args.depth)
                params['depth'] = String(args.depth);
            if (args.fields)
                params['fields'] = String(args.fields);
            if (args.limit)
                params['limit'] = String(args.limit);
            if (args.offset)
                params['offset'] = String(args.offset);
            if (args.include_meta === false)
                params['include-meta'] = 'false';
        ...
        case 'nsp_inventory_get_network_element': {
            const neId = args.ne_id;
            const params = {};
            if (args.depth)
                params['depth'] = String(args.depth);
            return client.get(`/restconf/data/nsp-equipment:network/network-element=${encodeURIComponent(neId)}`, params);
        }
        ...
        case 'nsp_inventory_get_shelves': {
            if (args.ne_id) {
                return client.get(`/restconf/data/nsp-equipment:network/network-element=${encodeURIComponent(args.ne_id)}/hardware-component/shelf`);
            }
        ...
        case 'nsp_inventory_get_cards': {
            ...
            if (args.offset)
                params['offset'] = String(args.offset);
            if (args.ne_id) {
                return client.get(`/restconf/data/nsp-equipment:network/network-element=${encodeURIComponent(args.ne_id)}/hardware-component/card`, params);
            }
            if (!args.limit)
                params['stream'] = '';
            return client.get('/restconf/data/nsp-equipment:network/network-element/hardware-component/card', params);
        }
        case 'nsp_inventory_get_ports': {
            ...
            if (args.offset)
                params['offset'] = String(args.offset);
            if (args.ne_id) {
                return client.get(`/restconf/data/nsp-equipment:network/network-element=${encodeURIComponent(args.ne_id)}/hardware-component/port`, params);
            }
            if (!args.limit)
                params['stream'] = '';
            return client.get('/restconf/data/nsp-equipment:network/network-element/hardware-component/port', params);
        }
        case 'nsp_inventory_get_port': {
            const neId = encodeURIComponent(args.ne_id);
        ...
        case 'nsp_inventory_get_fans': {
            if (args.ne_id) {
                return client.get(`/restconf/data/nsp-equipment:network/network-element=${encodeURIComponent(args.ne_id)}/hardware-component/fan`);
            }
            return client.get('/restconf/data/nsp-equipment:network/network-element/hardware-component/fan', { stream: '' });
        }
        case 'nsp_inventory_get_power_supplies': {
            if (args.ne_id) {
                return client.get(`/restconf/data/nsp-equipment:network/network-element=${encodeURIComponent(args.ne_id)}/hardware-component/power-supply`);
            }
            return client.get('/restconf/data/nsp-equipment:network/network-element/hardware-component/power-supply', { stream: '' });
        }
        default:
