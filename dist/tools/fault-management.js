...
export function getFaultManagementTools() {
    return [
        // ── Active Alarms ──────────────────────────────────────────────────────
        {
            ...
            description: 'Get active alarms from NSP Fault Management. Supports filtering by severity, NE, alarm name, and other criteria.',
            inputSchema: {
                type: 'object',
                properties: {
                    alarm_filter: {
                        type: 'string',
                        description: 'SQL-like filter expression. Examples: "severity=\'major\'", "alarmName like \'%LinkDown%\'", "(severity=\'critical\' or severity=\'major\') and impact > 0"',
                    },
                    include_details: {
                        type: 'boolean',
        ...
        },
        {
            ...
            description: 'Get aggregated alarms for a specific object (e.g., a Network Element or Equipment) by its FDN.',
            inputSchema: {
                type: 'object',
                required: ['object_fdn'],
                properties: {
                    object_fdn: {
                        type: 'string',
                        description: 'Object FDN, e.g. "fdn:model:equipment:NetworkElement:17644559" or "fdn:model:equipment:Equipment:17769693"',
                    },
                },
            },
        },
        ...
        // ── Alarm Settings ─────────────────────────────────────────────────────
        {
            ...
            description: 'Get alarm management settings: acknowledgement, aging, deletion, historical archive, overflow, or severity settings.',
            inputSchema: {
                type: 'object',
...
export async function handleFaultManagementTool(name, args, client) {
    const FM_BASE = '/FaultManagement/rest/api/v2';
    const encodeFdn = (fdn) => encodeURIComponent(fdn);
    switch (name) {
        case 'nsp_fm_get_alarms': {
            ...
            if (args.page_start !== undefined)
                params['pageStart'] = String(args.page_start);
            if (args.page_end !== undefined)
                params['pageEnd'] = String(args.page_end);
        ...
        case 'nsp_fm_get_alarm': {
            const fdn = encodeFdn(args.alarm_fdn);
        ...
        case 'nsp_fm_get_aggregated_alarms': {
            const fdn = encodeFdn(args.object_fdn);
...
//# sourceMappingURL=fault-management.js.map